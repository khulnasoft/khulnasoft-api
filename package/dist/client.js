...
exports.PaginatorPromise = exports.ClientPromise = exports.PageImpl = void 0;
exports.guessRequestEndpoint = guessRequestEndpoint;
exports.createClient = createClient;
const qs_1 = __importDefault(require("qs"));
const createRecursiveProxy_1 = require("./createRecursiveProxy");
...
function guessRequestEndpoint(baseUrl, callPath, action) {
    return `${actionMethod(action)} ${baseUrl}/${callPath.join("/")}`;
}
/**
 * Creates a client for making requests against a `khulnasoft` framework API.
 * For usage information see {@link KhulnasoftClient}.
 *
 ...
 * ```ts
 * // ~/api/client.ts
 * import { createClient } from "khulnasoft";
...
function createClient(baseUrl, options) {
    ...
    const client = (0, createRecursiveProxy_1.createRecursiveProxy)((opts) => {
        ...
        if (query && Object.keys(query).length) {
            search = `?${qs_1.default.stringify(query)}`;
            uri += search;
        }
        let cacheKey = uri;
        if (action === "list") {
            const { pageSize, sortDirection = "asc" } = query, restQuery = __rest(query, ["pageSize", "sortDirection"]);
            cacheKey = `${pathname}?${qs_1.default.stringify(Object.assign(Object.assign({}, restQuery), { sortDirection }))}`;
        }
        const doFetch = () => __awaiter(this, void 0, void 0, function* () {
            ...
            const parsed = khulnasoft_1.z.AnyPageData.safeParse(json);
            if (parsed.success) {
...
class PageImpl {
     ...
     */
    getPreviousPageUrl() {
        return `${this.pathname}/${qs_1.default.stringify(this.getPreviousPageParams())}`;
    }
    /**
     ...
     */
    getNextPageUrl() {
        return `${this.pathname}/${qs_1.default.stringify(this.getNextPageParams())}`;
    }
    /**
     * Gets the next page.
     * @throws if there is no `endCursor`.
...
exports.PageImpl = PageImpl;
/** A promise returned by {@link KhulnasoftClient} requests. */
class ClientPromise {
    constructor(fetch, props) {
        this.fetch = (0, lodash_1.once)(fetch);
 ...
 * all items.
 */
class PaginatorPromise extends ClientPromise {
    constructor(fetch, props) {
        super(fetch, props);
    }
