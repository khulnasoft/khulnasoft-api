...
commander_1.program.addOption(
  new commander_1.Option("-m, --module-type")
...
function main() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (!packageJsonPath) {
      console.error(
        `Folder ${path_1.default.relative(".", options.directory)} and its parent directories do not contain a package.json.`,
      );
      process.exit(1);
    }
    const rootPath = path_1.default.dirname(packageJsonPath);
    const tsConfigFilePath = path_1.default.relative(
      ".",
      path_1.default.join(rootPath, "tsconfig.json"),
    );
    ...
    const generationConfig = (0, filePathConfig_1.createGenerationConfig)(
      generationOptions,
    );
    let watcher;
    if (options.watch) {
    ...
    const project =
      (watcher === null || watcher === void 0 ? void 0 : watcher.project) ||
      new tm.Project({
    ...
    if (watcher) {
        ...
        __awaiter(this, void 0, void 0, function* () {
          ...
          try {
            ...
            while (evaluateRequested) {
              ...
              if (!debug_1.debug.enabled) console.clear();
              ...
              try {
                const succeeded = yield evaluate(
                  project,
                  baseCtx,
                  generationConfig,
                  printer,
      ...
      try {
        for (
          var _d = true, _e = __asyncValues(watcher.getEvents()), _f;
...
function evaluate(
  ...
  return __awaiter(
    ...
    function* (
      ...
      // every khulnasoft.types.endpoint call found per file
      ...
      for (const file of project.getSourceFiles()) {
        ...
        for (const callExpression of file.getDescendantsOfKind(
          ts_morph_1.ts.SyntaxKind.CallExpression,
        )) {
          ...
          if (methodName == "endpoint") {
            ...
            const name = (0, utils_1.mangleRouteToIdentifier)(
              call.endpointPath,
            );
          ...
          // Handle khulnasoft.codegenSchema call
          ...
          if (
            ...
            try {
              ...
              if (e instanceof convertType_1.ErrorAbort) break;
              else throw e;
            } finally {
              addDiagnostics(ctx, file, callExpression, callDiagnostics);
            }
            ...
            const declarationFilePath = declaration
              .getSourceFile()
              .getFilePath();
            ...
            try {
              ...
              schemaExpression = (0, ts_to_zod_1.convertType)(
                ctx,
                type,
                diagnosticItem,
              );
          ...
          fileOperations.push(() =>
            callExpression.addArgument(
              printer.printNode(
                ts_morph_1.ts.EmitHint.Unspecified,
                schemaExpression,
                file.compilerNode,
              ),
            ),
        ...
        if (fileInfo) {
          for (const [name, importInfo] of fileInfo.imports) {
            ...
            if (!importInfo.excludeFromUserFile) {
              imports.set(
                name,
                Object.assign(Object.assign({}, importInfo), {
        ...
        let importDeclarations = (0, generateFiles_1.generateImportStatements)(
          generationConfig,
          file.getFilePath(),
          imports,
          namespacedImports,
        ...
        for (const [idx, statement] of file.getStatements().entries()) {
          if (statement instanceof tm.ImportDeclaration) {
            ...
            const moduleSpecifier = statement
              ...
              .getLiteralValue();
            const sourcePath =
              (_a = statement.getModuleSpecifierSourceFile()) === null ||
              _a === void 0
                ? void 0
                : _a.getFilePath();
            if (
              (sourcePath === null || sourcePath === void 0
                ? void 0
            ...
            } else if (moduleSpecifier === "khulnasoft") {
        ...
        for (const declaration of importDeclarations) {
          ...
          const importDeclarationReference =
            currentImports.get(moduleSpecifier);
      ...
      const generatedFileContents = (0, generateFiles_1.generateFiles)(
        baseCtx,
        generationConfig,
      );
        ...
        for (const { filePath, line, column, diagnostics } of callDiagnostics) {
          output.push(
            chalk_1.default.magenta(
              `While processing codegenSchema call at ${path_1.default.relative(".", filePath)}:${line}:${column}:`,
            ),
          );
          for (const [filePath, fileDiagnostics] of diagnostics) {
            ...
            for (const warning of fileDiagnostics.warnings) {
              output.push(
                generateIncidentLocation(
                  path_1.default.relative(".", filePath),
                  warning,
                ),
              );
              output.push(
      ...
      if (endpointCalls.size) {
        const mapEntries = [];
        const endpointMapGenPath = path_1.default.join(
          generationConfig.basePath,
          "index.ts",
        );
        for (const [file, calls] of endpointCalls) {
          for (const call of calls) {
            const mangledName = (0, utils_1.mangleRouteToIdentifier)(
              call.endpointPath,
            );
            const importExpression = factory.createCallExpression(
              factory.createToken(ts_morph_1.ts.SyntaxKind.ImportKeyword),
              undefined,
              [
                factory.createStringLiteral(
                  `${(0, generateFiles_1.relativeImportPath)(endpointMapGenPath, (0, utils_1.convertPathToImport)((0, generateFiles_1.generatePath)(file.getFilePath(), generationConfig)))}${generationConfig.moduleType === "module" ? ".js" : ""}`,
                ),
              ],
            );
        ...
        yield fs_1.default.promises.mkdir(generationConfig.basePath, {
          recursive: true,
        });
        yield fs_1.default.promises.writeFile(
          endpointMapGenPath,
          yield (0, format_1.format)(
      ...
      for (const [file, fileStatments] of generatedFileContents) {
        ...
        yield fs_1.default.promises.writeFile(
          file,
          yield (0, format_1.format)(printer.printFile(sourceFile), file),
        );
      }
      project.save();
      return true;
...
function convertEndpointType(
  ...
  if (
    ...
    const symbol = typeArgument.getTypeName().getSymbolOrThrow();
    const schema = (0, convertType_1.convertSymbol)(
      ctx,
      symbol,
      diagnosticItem,
      type,
