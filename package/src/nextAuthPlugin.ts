...
import { NextServerContext } from "@khulnasoft-api/next";
...
export const makeNextAuthPlugin =
  ({ authOptions }: { authOptions: AuthOptions }): MakeKhulnasoftPlugin<any> =>
  (khulnasoft) => ({
    async middleware<EC extends AnyEndpoint>(
      ...
      // and somehow signal to khulnasoft.execute to early
      // exit
