...
# Copyright (C) 2015-2016 Khulnasoft, Inc.All rights reserved.
# Khulnasoft.com
...
error_and_exit() {
    ...
    if [ -d "/root/khulnasoft-api-tmp" ]; then
        exec_cmd "rm -rf /root/khulnasoft-api-tmp"
    fi
...
previous_checks() {
    ...
    if [ "X${arg}" == "Xdependencies" ]; then  # dependencies argument
        ...
        API_SOURCES="/root"
        DOWNLOAD_PATH=$(url_latest_release "khulnasoft" "khulnasoft-api")
    else
    ...
    if ! [ -d $FRAMEWORK_PATH ]; then
        print "Can't find $FRAMEWORK_PATH. Is Khulnasoft installed?.\nExiting."
        exit 1
    fi
    ...
    if [ $NODE_VERSION -lt 4 ]; then
        print "The current version of NodeJS installed is not supported. Khulnasoft API requires NodeJS 4.x or newer."
...
get_api () {
    if [ "X$DOWNLOAD_PATH" != "X" ]; then
        ...
        if [ -d "$API_SOURCES/khulnasoft-api-tmp" ]; then
            exec_cmd "rm -rf $API_SOURCES/khulnasoft-api-tmp"
        fi
        exec_cmd "mkdir -p $API_SOURCES/khulnasoft-api-tmp"

        exec_cmd "curl -sL $DOWNLOAD_PATH | tar xvz -C $API_SOURCES/khulnasoft-api-tmp"

        API_SOURCES="$API_SOURCES/khulnasoft-api-tmp/khulnasoft-api-*.*"

        API_NEW_VERSION=`cat $API_SOURCES/package.json | grep "version\":" | grep -P "\d+(?:\.\d+){0,2}\-*\w*" -o`
        print "\nInstalling Khulnasoft API $API_NEW_VERSION"
    else
        API_NEW_VERSION=`cat $API_SOURCES/package.json | grep "version\":" | grep -P "\d+(?:\.\d+){0,2}\-*\w*" -o`
        if [ "X${arg}" == "Xdev" ]; then
            print "\nInstalling Khulnasoft API $API_NEW_VERSION from current directory [DEV MODE]."
        else
            print "\nInstalling Khulnasoft API $API_NEW_VERSION from current directory."
        fi
    fi
...
setup_api_permissions () {
    ...
    exec_cmd "chmod ugo-x $API_PATH/scripts/khulnasoft-api*"
...
setup_api() {
    ...
    if [ -d $API_PATH ]; then
        ...
        if [[ "X${REINSTALL}" != "X" ]]; then
            ...
            while true; do
                read -p "Khulnasoft API is already installed (version $API_OLD_VERSION). Do you want to update it? [y/n]: " yn
                case $yn in
                    [Yy] ) update="yes"; break;;
                    [Nn] ) update="no"; break;;
                esac
    ...
    if [ "X${arg}" == "Xdev" ]; then
        ...
        # Set up the right permissions for Khulnasoft API
        setup_api_permissions
