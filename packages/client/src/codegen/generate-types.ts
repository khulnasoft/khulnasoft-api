...
import { AnyActionsConfig, ResourceConfig, z } from "khulnasoft";
import { splitPathIntoParts } from "../core/endpoint";
import { camelCase, capitalize } from "../util/strings";
import { ZodTypeAny } from "khulnasoft/dist/z";
...
function makeTypes(
  ...
  if (config.extensions) {
    output.push(dedent`
      ...
      type KhulnasoftApiProvidedOpts = "queryFn" | "queryKey" | "mutationFn";
      type UseQueryOptions = Omit<ReactQuery.UseQueryOptions, KhulnasoftApiProvidedOpts>;
      type UseSuspenseQueryOptions = Omit<ReactQuery.UseSuspenseQueryOptions, KhulnasoftApiProvidedOpts>;
...
export async function generateOutput<API extends APIConfig>(
  api: API,
  config: ClientConfig,
  installLocation: string = "@khulnasoft-api/client",
