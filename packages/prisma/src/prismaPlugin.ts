...
declare module "zod" {
  interface ZodType<Output, Def extends ZodTypeDef, Input = Output> {
     ...
     * is determined by the `prismaModel` passed to `khulnasoft.response` or
     * `z.pageResponse`.
     */
...
export type extractPrismaModel<T extends z.ZodTypeAny> = z.extractDeepMetadata<
  T,
  { khulnasoft: { prismaModel: () => PrismaHelpers } }
> extends { khulnasoft: { prismaModel: () => infer P extends PrismaHelpers } }
  ? P
  : never;
...
export function extractPrismaModel<T extends z.ZodTypeAny>(
  ...
  if (metadata) {
    return (metadata as any)?.khulnasoft?.prismaModel();
  }
...
export type PrismaContext<M extends PrismaHelpers> = {
  /**
   * The `prismaModel` passed to `khulnasoft.response` or
   * `z.pageResponse`.
   */
  prismaModel: M;
  /**
...
export const makePrismaPlugin =
  (): MakeKhulnasoftPlugin<PrismaStatics> => (khulnasoft) => {
    return {
      statics: {
        pagination: {
          wrapQuery,
...
type MakePrismaModelMetadata<M> = { khulnasoft: { prismaModel: () => M } };
