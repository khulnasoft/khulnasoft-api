...
export type isIncludable<T extends z.ZodTypeAny> = extractDeepMetadata<
  T,
  { khulnasoft: { includable: true } }
> extends { khulnasoft: { includable: true } }
  ? true
  : false;
 ...
 */
class KhulnasoftSelectable<T extends z.ZodTypeAny> extends z.ZodOptional<T> {
    ...
    return this._def.schema
      ...
      .then((base: any) => {
        ...
        return Promise.resolve(
          effect.transform(base.value, khulnasoftContext, {
            data: base.value,
            path: input.path,
            parent: input.parent,
          })
...
export type isPageResponse<T extends z.ZodTypeAny> = extractMetadata<
  T,
  { khulnasoft: { pageResponse: true } }
> extends { khulnasoft: { pageResponse: true } }
  ? true
  : false;
...
export type toZod<T> = 0 extends 1 & T
  ...
  ? // @ts-ignore: every branch of toZod returns a ZodTypeAny, but TS (5.9+)
    // with zod 3.25+ exhausts its instantiation depth budget trying to prove
    // the ZodOptional/ZodNullable generic constraints for unresolved T.
    // Concrete instantiations are still fully checked at use sites.
...
> {
  ...
  declare metadata: { khulnasoft: { includable: true } };
}
...
> {
  ...
  declare metadata: { khulnasoft: { selectable: true } };
}
...
> {
  ...
  declare metadata: { khulnasoft: { selects: true } };
}
