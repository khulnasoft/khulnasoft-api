...
async function evaluate(
  ...
  for (const file of project.getSourceFiles()) {
    ...
    for (const callExpression of file.getDescendantsOfKind(
      ...
      if (!symbol || !isSymbolKhulnasoftMethod(symbol)) continue;
  ...
  if (endpointCalls.size) {
    ...
    // The lazy dynamic imports below intentionally use extensionless
    // specifiers so the same file works when bundled (webpack
    // extensionAlias), transpiled to CommonJS (ts-node/jest), or loaded
    // natively. That form is a type error under moduleResolution
    // node16/nodenext (TS2835), so checking of this generated file is
    // disabled; the specifiers are computed, not hand-written.
    const mapSourceText = `// @ts-nocheck: generated file - the lazy dynamic imports below
// intentionally use extensionless specifiers (see packages/cli/src/index.ts)
${printer.printFile(mapSourceFile)}`;
    await fs.promises.writeFile(
      endpointMapGenPath,
      await format(mapSourceText, endpointMapGenPath)
    );
  }
