...
- [@khulnasoft-api/express: Express integration for Khulnasoft API](#khulnasoft-apiexpress-express-integration-for-khulnasoft-api)
- [Table of Contents](#table-of-contents)
- [Getting started](#getting-started)
  - [Installation](#installation)
  - [Creating an Express Router](#creating-an-express-router)
    - [From an entire Khulnasoft API](#from-an-entire-khulnasoft-api)
      - [Options](#options)
      - [Notes](#notes)
    - [From a Khulnasoft Resource](#from-a-khulnasoft-resource)
      - [Options](#options-1)
      - [Notes](#notes-1)
  - [Lower-level APIs](#lower-level-apis)
    - [Create an Express request handler](#create-an-express-request-handler)
      - [Options](#options-2)
      - [Notes](#notes-2)
    - [Call a Khulnasoft API endpoint from an Express handler](#call-a-khulnasoft-api-endpoint-from-an-express-handler)
...
```ts
import { apiRouter } from "@khulnasoft-api/express";
...
```ts
import { resourceRouter } from "@khulnasoft-api/express";
import express from "express";
...
```ts
import express from "express";
import { khulnasoftExpressRouteHandler } from "@khulnasoft-api/express";
...
By default `khulnasoftExpressRouteHandler` will handle all errors on the route.
If you want to customize error handling you may want to pass the
`handleErrors: false` option.

### Call a Khulnasoft API endpoint from an Express handler

```ts
khulnasoftExecuteExpressRequest<EC extends AnyEndpoint>(
  endpoint: AnyEndpoint,
  req: Request,
  res: Response
...
import { khulnasoftExecuteExpressRequest } from "@khulnasoft-api/express";
import retrievePosts from "./posts/retrieve";
...
```ts
khulnasoftPrepareExpressRequest<EC extends AnyEndpoint>(
  endpoint: AnyEndpoint,
  req: Request,
  res: Response
): Promise<[RequestData<EC["path"], EC["query"], EC["body"]>, KhulnasoftContext<EC>]>
```

```ts
import express, { Request, Response } from "express";
import { khulnasoftPrepareExpressRequest } from "@khulnasoft-api/express";
import retrievePosts from "./posts/retrieve";
