...
export interface ImportInfo {
  ...
  /** Whether this import shouldn't be imported into a user file when using `khulnasoft.codegenSchema`. */
...
export function convertType(
  ...
  if (isKhulnasoftSymbol(typeSymbol)) {
...
function isKhulnasoftSymbol(symbol: tm.Symbol | undefined): boolean {
  return isSymbolInFile(symbol, /khulnasoft\/dist\/.*\.d\.ts$/);
}

function isZodSymbol(symbol: tm.Symbol | undefined): boolean {
  return (
    isSymbolInFile(symbol, /zod\/lib\/.*\.d\.ts$/) || isKhulnasoftSymbol(symbol)
  );
}

function isTsToZodSymbol(symbol: tm.Symbol | undefined): boolean {
  return isKhulnasoftSymbol(symbol);
}

function isKhulnasoftPrismaSymbol(symbol: tm.Symbol | undefined): boolean {
...
function convertIncludesSelects(
  ...
  if (args.length < 1 || args.length > 2) {
    throw new Error("khulnasoft takes one or two type parameters");
  }
