...
const setupGitIdentity = _.once(async () => {
  if (process.env.CI) {
    await execa(
      "git",
      ["config", "--global", "user.email", "dev@khulnasoft.com"],
      {
        stdio: "inherit",
      }
    );
    await execa("git", ["config", "--global", "user.name", "Khulnasoft Bot"], {
      stdio: "inherit",
    });
  }
});
...
export async function gitPublish(packageDir, options) {
  ...
  const branch = tarball.replace(/^khulnasoft-api-/, "").replace(/\.tgz$/, "");
