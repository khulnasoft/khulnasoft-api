...
See [`khulnasoft` package docs](/khulnasoft/getting-started) to get started!

## Packages

- [khulnasoft](/khulnasoft/getting-started)
- [@khulnasoft-api/next](/khulnasoft/next/getting-started)
- [@khulnasoft-api/next-auth](/khulnasoft/next-auth/getting-started)
- [@khulnasoft-api/prisma](/khulnasoft/prisma/getting-started)
...
```ts
const khulnasoft = new Khulnasoft({ pristine: true });
```
...
- [Pagination](/khulnasoft/pagination#pristine-convention)
- [Inclusion](/khulnasoft/inclusion)
- [Selection](/khulnasoft/selection#pristine-convention)
...
const create = khulnasoft.endpoint({
  endpoint: "POST /users",
  response: User,
...
```ts
// app/api.ts
const users = khulnasoft.resource({
  models: { User },
  actions: {
    create,
  },
...
app.use((err, req, rsp, next) => {
  if (err instanceof khulnasoft.Error) {
    rsp.status(err.statusCode).send(khulnasoft.makeError(err));
  }
  // …
});
```
