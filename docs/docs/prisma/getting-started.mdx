...
available in its `handler`. `ctx.prisma` provides wrappers for the following
methods that magically inject options for [`include`](/khulnasoft/inclusion) and
[`select`](/khulnasoft/selection) params as necessary:

- `findUnique`
- `findUniqueOrThrow`
- `findMany` (also magically
  [injects pagination options](/khulnasoft/prisma/pagination#lower-level-ctxprismafindmanyoptions)
  from parameters)
- `create`
- `update`
- `delete`
...
The following `post: PostIdLoader` parameter takes a string `id` as input from
the `{post}` path parameter and is transformed to the fetched model instance in
the `handler`.
