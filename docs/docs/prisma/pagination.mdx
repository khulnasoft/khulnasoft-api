...
At the moment, `@khulnasoft-api/prisma` doesn't support multi-column sort order
in pagination.

Also, `@khulnasoft-api/prisma` current treats `sortBy` as a column name and uses
it to determine the cursor of each row.
...
If your endpoint `response` declares a `z.PrismaModel<>`, you will be able to
...
We plan to make [`z.PageResponse<>`](/khulnasoft/pagination#tpageresponseitem)
...
`ctx.prisma.findMany()` will inject `limit: pageSize + 1` so that it's possible
to > determine `hasPreviousPage` or `hasNextPage` by the presence or absence of
an extra item in the query results. If the extra item is present it should not
be included in the response, and will be removed by `makeResponse()`.
...
a [z.PageData](/khulnasoft/pagination#zpagedatai) response.
...
Creates a [`z.PageData`](/khulnasoft/pagination#zpagedatai) response from the
given items.
