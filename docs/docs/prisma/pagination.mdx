...
parameters) and resolves to a [z.PageData](/khulnasoft/pagination#zpagedatai) response.
...
Creates a [`z.PageData`](/khulnasoft/pagination#zpagedatai) response from the given items.
