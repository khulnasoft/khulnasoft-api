...
We provide two ways of converting schemas to types. The first is `khulnasoft.types<>().endpoint()`,
which converts its type parameters to Zod schemas. The second is `khulnasoft.codegenSchema<>()`, which our
...
[`khulnasoft`](/khulnasoft/cli) utilizes the Typescript compiler's type-checking API to enable powerful type analysis.
...
:::note
Whenever input types to `khulnasoft.types` or `khulnasoft.codegenSchema` change, the CLI needs
...
In order to use `khulnasoft.types.endpoint`, you need to add generated
schema generation to your instance of the `Khulnasoft` class:
...
Ordinary Typescript types can be extended with types provided by the `khulnasoft` package to add
...
`khulnasoft` provides the `*Schema` family of types for specifying primitive Zod validation rules
...
In total, `khulnasoft` offers the `StringSchema`, `NumberSchema`, `BigIntSchema`, `DateSchema`,
...
`khulnasoft.codegenSchema` will, as necessary, generate imports in the file it's called
