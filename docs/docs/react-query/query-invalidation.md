...
`client.<Resource>.<Action>.getQueryKey([path], [query])` to get the `queryKey`
to pass to `QueryClient.invalidateQueries`.

In the future, we plan to add methods like
`client.<Resource>.invalidateQueries()` and
