...
For every `get` endpoint in your API, there will be a `use<Action>()` method on
the Khulnasoft React Query client that wraps
...
The signature of `use<Action>` methods varies depending on whether the endpoint
...
The last argument is the `useQuery` options (except for `queryKey`, `queryFn`,
`getNextPageParam` and `getPreviousPageParam`, which are managed by the
Khulnasoft React Query client).
