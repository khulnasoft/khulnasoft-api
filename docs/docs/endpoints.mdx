...
A Khulnasoft API endpoint declares a method and URL, request and response parameter schemas, and
a handler function that handles the request. Endpoints are grouped into [resources](/khulnasoft/resources),
...
`khulnasoft.types` is a helper that enables generating rich, validating schemas for the parameter and
response types of the endpoint. After running the `@khulnasoft-api` CLI, these type definitions are
...
For more information about the CLI, visit its [dedicated page](/khulnasoft/cli).

`khulnasoft.types` accepts one type parameter, which is used to specify the types of the parameters accepted by the endpoint,
...
`khulnasoft` provides some exception types to streamline this. Each will cause the endpoint to respond with an HTTP status code.
...
This method takes the same properties as the `endpoint` method available when using `khulnasoft.types`,
...
The [schema](/khulnasoft/schemas/runtime-zod-schemas) for the query (search) parameters.

### `body`

The [schema](/khulnasoft/schemas/runtime-zod-schemas) for the request body parameters.

## Path and query parameter coercion

Since the raw path and query parameters from an HTTP request are strings, if you
provide a number, boolean, Date, or bigint schema for a parameter, Khulnasoft will
automatically coerce the raw value to that type.

To accomplish this Khulnasoft deeply maps the parameter schemas, adding a coercion
