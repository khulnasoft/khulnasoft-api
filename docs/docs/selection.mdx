...
The [pristine convention](/khulnasoft/intro#pristine) is to have a selectable property as a sibling of an
[includable](/khulnasoft/inclusion) property. So in the above example
...
`include` for prisma queries via [`ctx.prisma`](/khulnasoft/prisma/getting-started#perform-crud-operations-on-response-prismamodel) or [`PrismaModelLoader`](/khulnasoft/prisma/getting-started#use-prismamodelloader-on-a-parameter) when
you have [declared a `PrismaModel`](/khulnasoft/prisma/getting-started#declare-prismamodel-on-a-response-type) on the response schema.
