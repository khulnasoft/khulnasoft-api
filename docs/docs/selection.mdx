...
The [pristine convention](/khulnasoft/intro#pristine) is to have a selectable
property as a sibling of an [includable](/khulnasoft/inclusion) property. So in
the above example there would be an includable `user` property, whose type has
some properties always defined, and a selectable `user_fields` property, whose
type has all properties optional.
...
for `select` in the future to achieve better type safety (for example, the same
format as `include`).
...
You can add support for a selectable field by marking it `z.Selectable<>` in the
response schema and adding `select: z.Selects<>` to your query schema:

:::caution

Currently, the `z.Selects<>` parameter must be named `select` and declared in
the `query`.
...
Although `z.Selectable<>` can be can be instantiated with any type, it only
works with schemas of type object or array of objects.
...
query parameter from the response properties that are marked `z.Selectable<>`
...
[`ctx.prisma`](/khulnasoft/prisma/getting-started#perform-crud-operations-on-response-prismamodel)
or
[`PrismaModelLoader`](/khulnasoft/prisma/getting-started#use-prismamodelloader-on-a-parameter)
when you have
[declared a `PrismaModel`](/khulnasoft/prisma/getting-started#declare-prismamodel-on-a-response-type)
...
When using Zod schemas with circular associations, it becomes necessary to
declare types for the schemas due to TypeScript limitations. Khulnasoft provides
...
:::caution The maximum recursion depth supported by `z.includes` is 5.
